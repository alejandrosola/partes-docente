...
@Service
public class Articulo23A implements ArticuloControl {
    ...
    public void checkValid(Licencia aLicencia, List<Licencia> someLicencias) {
        ...
        if (count > 30) {
            throw new DataIntegrityViolationException(String.format(
                    "NO se otorga Licencia artículo 23A a %s %s debido a que supera el tope de 30 días de licencia",
    ...
    public boolean isResponsibleFor(ArticuloLicencia articulo) {
        return "23A".equals(articulo.getNombre());
    }
}